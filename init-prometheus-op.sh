...
oc process -f prometheus-operator/kafka-rules.yml -p NAMESPACE=kafka-1 -p KAFKA_SERVICE_TEAM=aims | oc apply -f -

oc apply -f prometheus-operator/burrow/burrow-deployment.yml -n kafka-1

# oc apply -f metrics/examples/grafana/kubernetes.yaml
# oc apply -f metrics/examples/grafana/grafana-config.yaml
# oc apply -f metrics/examples/grafana/grafana-datasrc.yaml
# oc apply -f metrics/examples/grafana/grafana-dashsrc.yaml
# oc apply -f metrics/examples/grafana/grafana-deploy.yaml
